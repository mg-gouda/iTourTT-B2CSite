...
export function RichEditor({
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class:
          'prose dark:prose-invert max-w-none min-h-[320px] px-4 py-3 focus:outline-none prose-headings:font-semibold prose-a:text-primary',
      },
    },
  ...
  const Btn = ({
    ...
    <button
      ...
      className={cn(
        ...
        active ? 'bg-primary/10 text-primary' : 'text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 hover:text-slate-900 dark:hover:text-slate-100',
      )}
    >
      {children}
    </button>
