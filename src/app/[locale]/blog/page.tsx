...
export async function generateMetadata({ params, searchParams }: Props): Promise<Metadata> {
  ...
  const base = await buildPageMetadata('blog', {
    ...
    fallbackDescription:
      'Travel tips, destination guides and airport transfer advice for your trip to Egypt. Updated regularly.',
  });
  if (page && parseInt(page, 10) > 1) {
    return { ...base, alternates: { canonical: `/${locale}/blog` } };
  }
  return base;
}
              ...
              {items.map((post) => (
                ...
                >
                  {post.coverImageUrl && (
                    ...
                    <img src={resolveAssetUrl(post.coverImageUrl)} alt={post.title} className="h-44 w-full object-cover" />
                  )}
                  <div className="flex flex-1 flex-col p-5">
                    ...
                    <h2 className="text-lg font-bold leading-snug text-gray-900 group-hover:text-emerald-700">{post.title}</h2>
                    {post.excerpt && (
                      <p className="mt-2 line-clamp-3 flex-1 text-sm text-gray-500">{post.excerpt}</p>
                    )}
            ...
            <div className="mt-12 flex items-center justify-center gap-2">
              {Array.from({ length: data.totalPages }, (_, i) => i + 1).map((p) => (
                <Link
                  ...
                  className={`rounded-md px-3 py-1.5 text-sm ${p === data.page ? 'bg-gray-900 text-white' : 'border border-gray-200 text-gray-600 hover:bg-gray-50'}`}
                >
                  {p}
                </Link>
              ))}
